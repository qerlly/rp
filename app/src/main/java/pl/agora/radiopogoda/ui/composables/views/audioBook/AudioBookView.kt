package pl.agora.radiopogoda.ui.composables.views.audioBook
...
import pl.agora.radiopogoda.ui.composables.views.pocast.ProgramCard
...
fun AudioBookView(
    ...
    LazyColumn(Modifier.fillMaxSize()) {
        when (val programs = mainUiState.value.audioPrograms) {
        ...
        when (val podcasts = mainUiState.value.audioPodcasts) {
            is ApiResult.Loading ->
                item {
                    Box(Modifier.fillMaxWidth().height(200.dp)) {
                        CircularProgressIndicator(
